...
def signup(userSignUp: schemas.UserSignUp, db: Session = Depends(db.get_db)):
    ...
    if (
        userSignUp.name == ""
        or userSignUp.email == ""
        or userSignUp.password == ""
        or userSignUp.role == ""
        or userSignUp.nid == ""
        or userSignUp.location == ""
        or userSignUp.phone == ""
    ):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Provide Necessary Details",
        )

    user_data = userSignUp.model_dump()
    user_data["password"] = utils.hash(userSignUp.password)
...
def forgetpassword(
    codeSchema: schemas.PasswordCodeSchema, db: Session = Depends(db.get_db)
):
    code = (
        db.query(models.PasswordCode)
        .filter(models.PasswordCode.email == codeSchema.email)
        .first()
    )
...
def confirmPassword(
    ...
    if code and codeSchema.code == code.code:
        db.delete(code)
        db.commit()
        return {"detail": "Success"}
    else:
...
def reset_password(
    resetPassword: schemas.ResetPasswordSchema, db: Session = Depends(db.get_db)
):
    user = (
        db.query(models.User)
        .filter(models.User.email == resetPassword.email)
        .first()
    )
    ...
    user.password = utils.hash(resetPassword.password)
    db.add(user)
    db.commit()
