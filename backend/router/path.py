...
def time_difference_in_minutes(time1: time, time2: time) -> int:
...
def get_path(
    ...
    if cached_routes_json:
        return json.loads(cached_routes_json)
    ...
    for route in routes:
        ...
        while True:
            ...
            current_route.path.append(
                PathSh1(
                    source_name=next_route.source.name if next_route.source else "",
                    destination_name=next_route.destination.name if next_route.destination else "",
                    ...
                    duration=time_difference_in_minutes(
                        next_route.leavetime, next_route.reachtime
                    ),
                )
            )
    ...
    result_routes_json = json.dumps(
        [r.model_dump() for r in result_routes], default=time_to_str
    )
